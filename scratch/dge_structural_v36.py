...
class StructuralDGEOptimizer:
    ...
    def step(self, f_batched, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        # Apply random signs to the structural blocks!
...
# K_BLOCKS Baseline = O(sqrt_D)
LAYER_SIZES   = [a * b + b for a, b in zip(ARCH[:-1], ARCH[1:])]
K_BLOCKS_BASE = [max(1, int(math.sqrt(D))) for D in LAYER_SIZES]
...
def run_dge(name, opt_cls, model, params0, X_tr, y_tr, X_te, y_te, seed):
    ...
    if opt_cls == TorchDGEOptimizer:
        total_steps = BUDGET_DGE // (2 * sum(K_BLOCKS_BASE))
        opt = opt_cls(
            dim=model.dim, layer_sizes=model.sizes, k_blocks=K_BLOCKS_BASE,
            lr=LR_ZO, delta=DELTA, total_steps=total_steps,
            consistency_window=20, seed=seed, device=device, chunk_size=128
        )
        evals_per_step = 2 * sum(K_BLOCKS_BASE)
    else:
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v36: Structural DGE (Node Perturbation)")
    ...
    METHODS = [
        ("DGE_Baseline_O(sqrt_D)", TorchDGEOptimizer),
        ("DGE_Structural", StructuralDGEOptimizer)
    ]
