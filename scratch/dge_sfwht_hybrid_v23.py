...
# =============================================================================
# HYBRID SFWHT + DGE ENGINE (V23 - High SNR & Randomized)
# =============================================================================
def fwht_tensor(a):
    n = a.shape[-1]
    h = 1
...
class HybridSFWHT_DGE_V23:
    def __init__(self, sizes, B_list, top_k_buckets, dge_blocks, explore_prob, total_steps, lr0=0.05, eps0=5e-3, delta0=1e-3):
        self.sizes = sizes
        self.B_list = B_list
    ...
    def estimate_layer_gradient(self, f_layer, x_layer, D_pad, actual_size, B, num_top_buckets, k_blocks, eps, delta, explore_p):
        # ==========================================
        ...
        # FIX 2: Random permutation before bucket mapping
        # Variable j will use column perm[j] of V_0, mapping it to bucket perm[j] % B
        ...
        # Map back to variables using the permutation
        bucket_ids = perm % B
        mask = torch.isin(bucket_ids, top_buckets)
        
        # FIX 3: Exploration for non-scanned variables
        explore_mask = torch.rand(actual_size, device=device) < explore_p
        ...
        if len(active_indices) > 0:
            # FIX 1: Increased DGE blocks (k_blocks) to drastically improve SNR
            active_perm = active_indices[torch.randperm(len(active_indices), device=device)]
            actual_k_blocks = min(k_blocks, len(active_perm))
    ...
    def step(self, f_eval, params):
        ...
        for i, (size, pad_size, B, num_top_buckets, k_blocks, explore_p) in enumerate(zip(
            ...
            g_layer, evs = self.estimate_layer_gradient(
                f_layer, x_layer, pad_size, size, B, num_top_buckets, k_blocks, eps, delta, explore_p
            )
...
if __name__ == "__main__":
    print("DGE v23: HYBRID SFWHT + DGE (High SNR, Randomized) on MNIST")
    ...
    # Hybrid Configuration v23:
    B_LIST = [512, 128]
    TOP_K_BUCKETS = [50, 12]
    DGE_BLOCKS = [128, 16]     # Drastically increased k blocks to reduce noise
    EXPLORE_PROB = [0.05, 0.05] # 5% chance to explore any parameter outside top buckets
    
    # Evals L1: 1024 (Scan) + 256 (Refine) = 1280
    # Evals L2: 256 (Scan) + 32 (Refine) = 288
    # Total Evals/Step: ~1568
    ...
    with open(f"results/raw/sfwht_hybrid_v23.json", "w") as f_out:
