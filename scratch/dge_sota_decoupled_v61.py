"""
dge_sota_decoupled_v61.py
=================================
Experiment: Mathematical Hyperparameter Scaling per Layer
Applies the decoupling lesson from v60 to a standard CONTINUOUS network.

Hypothesis: In standard networks, weights are initialized proportional to 
1/sqrt(fan_in). Deeper layers (with smaller fan_in) have larger weight 
magnitudes. Therefore, the perturbation delta and learning rate should 
scale mathematically with the layer's expected weight magnitude to ensure
uniform relative exploration.

Combines:
- Subdivided Neuron Architecture (Continuous)
- Dynamic Noise Budget
- Asymmetric Adam EMA & Hybrid Curvature (Megazord)
- Exponential LR Schedule
- PER-LAYER mathematically scaled Deltas and LRs.
"""
...
# ---------------------------------------------------------------------------
# Optimizer: SOTA Megazord + Dynamic Budget + Decoupled
# ---------------------------------------------------------------------------
class DecoupledDynamicOptimizer(TorchDGEOptimizer):
    def __init__(self, layer_lrs0, layer_lrs_min, layer_deltas0, *args, **kwargs):
        ...
        self._update_padding_logic()

    def _update_padding_logic(self):
        self.group_sizes = []
        self.pads = []
    ...
    def _recalculate_k_blocks(self):
        # Dynamic Noise Budget based on parameter variance
        noise_scores = []
        off = 0
    ...
    def step_sota(self, f_batched, x: torch.Tensor, sniff_ratio=0.1):
        ...
        # Note: we use a global normalized delta for the sniff to avoid directional bias.
        # We'll use the mean of the current layer deltas for the sniff radius.
        ...
        upd += grad_perp_s * (sum(self.layer_lrs0)/len(self.layer_lrs0)) # Use mean LR for sniff component
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------------------------
    # Standard deviation of initialization: ~ sqrt(2 / fan_in)
    scales = [math.sqrt(2.0 / l_in) for l_in in ARCH[:-1]]
    
    # Normalize scales so the FIRST layer has scale 1.0. 
    # This keeps L1 hyperparameters identical to the proven v57 baseline,
    # while deep layers get boosted Deltas and LRs.
    norm_scales = [s / scales[0] for s in scales]
    
    BASE_LR_MAX = 0.02     # From v57
    BASE_LR_MIN = 0.001    # From v57
    BASE_DELTA  = 1e-3     # From v57
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v61: MATHEMATICAL DECOUPLED SCALING (Continuous Net)")
    print(f"Scaling Rule: 1/sqrt(fan_in) relative to Layer 1")
    for i in range(len(ARCH)-1):
        print(f"  L{i+1} (in={ARCH[i]:>3}): Scale={norm_scales[i]:>4.2f}x | LR={LRS_0[i]:>6.4f} | DELTA={DELTAS[i]:>7.5f}")
    ...
    print(f"\n  [v61 SOTA Decoupled] evals/step = {2 * sum(k_blocks) + 2}")
    ...
    # Save results
    out_path = Path(__file__).parent.parent / "results" / "raw" / "v61_mathematical_scaling.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({
            "experiment": "v61_mathematical_scaling", 
            "seed": seed, 
            "acc": best_test,
