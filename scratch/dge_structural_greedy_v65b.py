"""
dge_structural_greedy_v65.py
=================================
Experiment to test GREEDY ALTERNATING Node Perturbation.
No EMA (No Adam). No learning rate.
Immediate updates: If a small additive or multiplicative perturbation
on a fan-block improves the loss, we apply it immediately.

- Sweeps alternate between ADDITIVE and MULTIPLICATIVE.
- In a sweep, we iterate over all structural blocks in a random order.
- For each block, we test +delta and -delta. If the best of these two 
  is better than the current loss, we accept the change immediately 
  and update our baseline.
"""
...
# ---------------------------------------------------------------------------
# Greedy Structural Optimizer
# ---------------------------------------------------------------------------

class GreedyStructuralOptimizer:
    def __init__(
        self,
        arch: list[int],
        dim: int,
        delta_add: float = 1e-4,  # Very small absolute change
        delta_mul: float = 1e-3,  # Very small relative change (1.001)
    ...
    def step_greedy(self, f_batched, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        delta = self.delta_add if is_additive else self.delta_mul
...
ARCH          = (784, 128, 64, 10)
BUDGET_DGE    = 2500_000
DELTA_ADD     = 1e-3  # Very small absolute additive step
DELTA_MUL     = 1e-3  # Multiplier is (1 +/- 1e-3)
BATCH_SIZE    = 256
SEEDS         = [42]
LOG_INTERVAL  = 50_000
...
def run_dge(name, opt_cls, model, params0, X_tr, y_tr, X_te, y_te, seed):
    ...
    opt = opt_cls(
        ...
        delta_add=DELTA_ADD, delta_mul=DELTA_MUL, 
        device=device
    )
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v65: Greedy Alternating Structural DGE")
    print(f"NO EMA. Immediate updates if Loss improves.")
    ...
    METHODS = [
        ("DGE_Greedy_Alternating", GreedyStructuralOptimizer)
    ]
    ...
    out_path = out_dir / "v65b_greedy_alternating_structural.json"
    
    payload = {
        "experiment": "v65b_greedy_alternating_structural",
