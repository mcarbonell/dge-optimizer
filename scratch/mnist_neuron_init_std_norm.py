...
if __name__ == "__main__":
    # Normalización ESTÁNDAR (Media 0)
    transform_std = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,))
    ])
    
    train_ds = datasets.MNIST('./data', train=True, download=True, transform=transform_std)
    test_ds = datasets.MNIST('./data', train=False, transform=transform_std)
    ...
    for mode_name, mode in [("POSITIVOS [0, 1]", "pos"), ("MIXTO [-1, 1]", "mix")]:
        print("\n" + "="*50)
        print(f"MODO: {mode_name} + NORMALIZACIÓN MEDIA-CERO")
        print("="*50)
