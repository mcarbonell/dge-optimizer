...
# =============================================================================
# RED NO DIFERENCIABLE (Step Activation)
# =============================================================================
...
def run_experiment(name, total_evals, type="dge", lr=0.5):
    print(f"\n>>> TESTING {name} (NON-DIFFERENTIABLE) | Budget: {total_evals} evals")
    ...
    if type == "dge":
        ...
        optimizer = DGEOptimizer(
            dim=D, lr=lr, delta=5e-3, # Slighly larger delta to encourage crossing the step boundary
    ...
    print(f"    FINAL {type.upper()}: Acc={final_acc:.2%} | Time={elapsed:.1f}s")
    return final_acc, elapsed

if __name__ == "__main__":
    print("DEMO: Training a model with STEP ACTIVATIONS (Non-Differentiable)")
    print("DGE expects to train it, while Adam/SGD expect to fail due to Zero Gradients.")
    
    # Run Adam (expected to fail)
    run_experiment("Adam Experiment", TOTAL_EVALS, type="adam")
    
    # Run DGE (expected to work)
    run_experiment("DGE Experiment", TOTAL_EVALS, type="dge", lr=0.5)
