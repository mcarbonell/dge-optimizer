...
def run_test(label, opt_type, params0, X_tr, y_tr, X_te, y_te, arch, budget, batch_size, seed):
    ...
    if opt_type == "SMA":
        opt = DGEOptimizer(dim=D, k_blocks=K, lr=0.1, total_steps=STEPS, consistency_window=20, seed=seed)
    else:
        opt = DualEMADGE_MNIST(dim=D, k_blocks=K, lr=0.1, total_steps=STEPS, seed=seed)
