"""
dge_canine_adam_v50.py
=================================
Experiment: Normalized Canine Sniffing.
To prevent learning rate collapse or explosions, we normalize the 
lateral sniffing gradient (grad_perp) relative to the main block 
gradient (grad_dge). 

The lateral "nudge" is scaled to be a fraction of the main signal.
"""
...
# ---------------------------------------------------------------------------
# Normalized Canine Optimizer
# ---------------------------------------------------------------------------
class NormalizedCanineOptimizer(TorchDGEOptimizer):
    def step_normalized(self, f_batched, x: torch.Tensor, sniff_ratio=0.1):
        self.t += 1
        ...
        if torch.norm(m_curr) > 1e-9:
            ...
            v_perp = r - (torch.dot(r, m_curr) / (torch.dot(m_curr, m_curr) + 1e-12)) * m_curr
        ...
        P_all[0:2*self.total_k:2] = P_plus; P_all[1:2*self.total_k:2] = -P_plus
        P_all[2*self.total_k:] = P_sniff
        ...
        L_sniff = losses[2*self.total_k:]
        grad_perp = ((L_sniff[0] - L_sniff[1]) / (2.0 * delta)) * v_perp
        
        # 4. NORMALIZATION: Scale lateral signal to sniff_ratio of main signal
        norm_dge = torch.norm(grad_dge)
        norm_perp = torch.norm(grad_perp)
        if norm_perp > 1e-9 and norm_dge > 1e-9:
            grad_perp_scaled = grad_perp * (norm_dge / norm_perp) * sniff_ratio
        else:
            grad_perp_scaled = torch.zeros_like(grad_perp)
            
        grad_total = grad_dge + grad_perp_scaled
        
        # 5. Adam Update (Safe Update)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v50: Normalized Canine Sniffing")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Mechanism: Lateral nudge = 10% of main gradient magnitude")
    print(f"{'='*70}")
    ...
    print(f"\n  [Normalized-Sniff] evals/step = {2 * sum(k_blocks) + 2}")
    ...
    while evals < BUDGET_DGE:
        ...
        params, n = opt.step_normalized(f_batched, params, sniff_ratio=0.1)
        evals += n
