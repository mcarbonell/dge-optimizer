"""
dge_foresight_v53.py
=================================
Experiment: Foresight & Local Backtracking
Instead of spending extra evaluations on lateral sniffing, we spend 1 extra 
evaluation to take a "test step" forward. If the loss degrades significantly 
compared to our local estimate (meaning we hit a valley wall), we automatically
reduce the step size (backtrack) for this iteration.
"""
...
# Foresight Optimizer
# ---------------------------------------------------------------------------
class ForesightOptimizer(TorchDGEOptimizer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.backtracks = 0

    def step_foresight(self, f_batched, x: torch.Tensor):
        self.t += 1
        ...
        # Proxy for current loss: mean of all perturbations
        L_curr_est = L_dge_all.mean().item()
        ...
        # 4. Foresight (Test step)
        x_prop = x - upd
        L_prop = f_batched(x_prop.unsqueeze(0))[0].item()
        
        # 5. Backtracking Decision
        # If the proposed step makes the loss MUCH worse than our local estimate,
        # it means we probably jumped out of the valley.
        # Note: In cross entropy loss, a higher value is worse.
        margin = abs(L_curr_est) * 0.05
        if L_prop > L_curr_est + margin:
            # Backtrack! Reduce step size
            upd *= 0.25 # Aggressive backtrack
            self.backtracks += 1
            x_next = x - upd
        else:
            x_next = x_prop
            
        return x_next, 2 * self.total_k + 1
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v53: Foresight & Local Backtracking")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Mechanism: Evaluates proposed step. If worse than local mean, reduces step by 75%.")
    print(f"{'='*70}")
    ...
    opt = ForesightOptimizer(
        dim=model.dim, layer_sizes=layer_sizes, k_blocks=k_blocks,
        lr=LR_ZO, delta=DELTA, total_steps=BUDGET_DGE//(sum(k_blocks)*2 + 1),
    ...
    print(f"\n  [v53 Foresight] evals/step = {2 * sum(k_blocks) + 1}")
    print(f"  {'evals':>10}  {'test_acc':>9}  {'best_test':>9}  {'time':>7}  {'bks':>5}")
    print(f"  {'-'*55}")
    ...
    while evals < BUDGET_DGE:
        ...
        params, n = opt.step_foresight(f_batched, params)
        evals += n

        if evals >= next_log or evals >= BUDGET_DGE:
            te_acc  = zo_acc(model, X_te_raw, y_te_d, params)
            best_test = max(best_test, te_acc)
            print(f"  {evals:>10,}  {te_acc:>8.2%}  {best_test:>8.2%}  {time.time()-t0:>6.0f}s  {opt.backtracks:>5}")
            next_log += LOG_INTERVAL
            
    # Save results
    out_path = Path(__file__).parent.parent / "results" / "raw" / "v53_foresight_backtracking.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({
            "experiment": "v53_foresight_backtracking", 
            "seed": seed, 
            "acc": best_test,
            "total_backtracks": opt.backtracks
        }, f, indent=2)
