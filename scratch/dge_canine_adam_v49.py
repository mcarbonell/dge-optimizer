"""
dge_canine_adam_v49.py
=================================
Experiment: Momentum-Only Canine Sniffing.
We apply the lateral sniffing signal only to the FIRST moment (m) of Adam.
The second moment (v) is updated only using the block-gradient to prevent 
learning rate collapse caused by lateral noise.
...
# ---------------------------------------------------------------------------
# Momentum-Only Canine Optimizer
# ---------------------------------------------------------------------------
class MomentumCanineOptimizer(TorchDGEOptimizer):
    def step_momentum_only(self, f_batched, x: torch.Tensor, alpha_sniff=1.0):
        self.t += 1
        ...
        # 4. Adam Update (Momentum-Only Sniff)
        # m uses both, v uses ONLY dge
        self.m = self.beta1 * self.m + (1.0 - self.beta1) * (grad_dge + alpha_sniff * grad_perp)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v49: Momentum-Only Canine Sniffing")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Mechanism: grad_perp added to m_t but NOT to v_t")
    print(f"{'='*70}")
    ...
    print(f"\n  [Momentum-Sniff] evals/step = {2 * sum(k_blocks) + 2}")
    ...
    while evals < BUDGET_DGE:
        ...
        params, n = opt.step_momentum_only(f_batched, params, alpha_sniff=1.0)
        evals += n
