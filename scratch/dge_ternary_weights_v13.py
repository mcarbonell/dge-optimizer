...
# =============================================================================
# MODELO TERNARIO (Weights in {-1, 0, 1})
# =============================================================================
class TernaryModel(nn.Module):
    ...
    def forward(self, x, ternary=True):
        ...
        for i, layer in enumerate(self.layers):
            w = layer.weight
            if ternary:
                # Ternary Clipping: -1 if x < -tau, 1 if x > tau, else 0
...
def run_experiment(name, total_evals, lr=0.5):
    print(f"\n>>> TESTING {name} (TERNARY WEIGHTS {-1, 0, 1}) | Budget: {total_evals} evals")
    ...
    print(f"    FINAL TERNARY DGE: Acc={final_acc:.2%} | Sparsity={final_sp:.1%} | Time={elapsed:.1f}s")
    return final_acc, elapsed

if __name__ == "__main__":
    print("DGE: Training a network where inference uses TERNARY weights {-1, 0, 1}")
    run_experiment("Ternary DGE", TOTAL_EVALS, lr=0.5)
