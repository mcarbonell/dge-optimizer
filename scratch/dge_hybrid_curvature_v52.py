"""
dge_hybrid_curvature_v52.py
=================================
Experiment: Híbrido Estructural-Curvatura + Adam Asimétrico

Combines:
1. Structural block-wise evaluation (from v45).
2. Curvature-guided lateral sniff, but computed independently per structural block!
   Instead of orthogonalizing the entire gradient globally, each block calculates
   its own Delta G and orthogonalizes it locally.
3. Asymmetric Adam: The lateral "sniff" signal (grad_perp) is NOT fed into Adam's 
   moments (m, v), preventing the "scared dog" collapse. It is added as a direct 
   correction to the parameter update.
"""
...
# ---------------------------------------------------------------------------
# Hybrid Curvature Canine Optimizer
# ---------------------------------------------------------------------------
class HybridCurvatureOptimizer(TorchDGEOptimizer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.g_prev = None

    def step_hybrid(self, f_batched, x: torch.Tensor, sniff_ratio=0.1):
        self.t += 1
        ...
        # 3. Hybrid Lateral Sniff (Block-wise Curvature)
        ...
        # Normalization globally after block-wise assignment
        n_v_perp = torch.norm(v_perp)
        if n_v_perp < 1e-9:
            # Fallback to random orthogonal block-wise
            off = 0
            for i, (sz, k, pad, grp) in enumerate(zip(self.layer_sizes, self.k_blocks, self.pads, self.group_sizes)):
                for j in range(k):
                    start = off + j * grp
        ...
        # 4. Normalization of lateral gradient
        ...
        # 5. Asymmetric Adam Update (only main gradient enters momentum)
        ...
        # Add lateral signal DIRECTLY to the update without polluting Adam's memory
        upd += lr * grad_perp_s
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v52: Híbrido Estructural-Curvatura + Adam Asimétrico")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Mechanism: Block-wise Delta G (Curvature) + Asymmetric Adam Injection")
    print(f"{'='*70}")
    ...
    print(f"\n  [v52 Hybrid] evals/step = {2 * sum(k_blocks) + 2}")
    ...
    while evals < BUDGET_DGE:
        ...
        params, n = opt.step_hybrid(f_batched, params, sniff_ratio=0.1)
        evals += n
    ...
    # Save results
    out_path = Path(__file__).parent.parent / "results" / "raw" / "v52_hybrid_curvature.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({"experiment": "v52_hybrid_curvature", "seed": seed, "acc": best_test}, f, indent=2)
