...
dge_megazord_v55.py
=================================
Experiment: The Megazord (Dynamic Budget + Hybrid Curvature)
Combines:
1. Dynamic Noise Budget (v54): Reallocates K blocks based on parameter variance.
2. Hybrid Curvature (v52): Local block-wise Delta G to sniff lateral walls.
3. Asymmetric Adam (v52): Lateral sniff only directly updates weights, not Adam's EMA.
"""
...
# Megazord Optimizer
# ---------------------------------------------------------------------------
class MegazordOptimizer(TorchDGEOptimizer):
    ...
    def step_megazord(self, f_batched, x: torch.Tensor, sniff_ratio=0.1):
        self.t += 1
        ...
        # 3. Hybrid Lateral Sniff (Block-wise Curvature from v52)
        ...
        # 5. Asymmetric Adam Update (only main grad_dge enters momentum)
        ...
        # Add lateral signal DIRECTLY to the update
        upd += lr * grad_perp_s
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v55: MEGAZORD (Dynamic Budget + Hybrid Curvature)")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Mechanism: Dynamic variance-based K allocation + Block-wise Curvature Sniff")
    print(f"{'='*70}")
    ...
    print(f"\n  [v55 Megazord] evals/step = {2 * sum(k_blocks) + 2}")
    ...
    while evals < BUDGET_DGE:
        ...
        params, n = opt.step_megazord(f_batched, params, sniff_ratio=0.1)
        evals += n
    ...
    # Save results
    out_path = Path(__file__).parent.parent / "results" / "raw" / "v55_megazord.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({
            "experiment": "v55_megazord", 
            "seed": seed, 
            "acc": best_test,
