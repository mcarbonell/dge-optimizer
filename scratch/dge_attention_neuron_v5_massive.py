...
class MassiveNeuronMLP(nn.Module):
    def __init__(self):
        super().__init__()
        # Red COLOSAL: 784 -> 4096 -> 4096 -> 10
        # Parametros de un MLP normal: ~20 Millones
        self.layer1 = UnifiedNeuronLayer(784, 4096)
        self.layer2 = UnifiedNeuronLayer(4096, 4096)
        self.layer3 = UnifiedNeuronLayer(4096, 10)
...
def main():
    ...
    print(f"Entrenando V5 (Massive) en: {device}")
    ...
    model = MassiveNeuronMLP().to(device)
    
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Parámetros entrenables: {total_params:,} (vs ~20,000,000 de un MLP estándar)")
