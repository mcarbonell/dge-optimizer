...
class MediumNeuronMLP(nn.Module):
    def __init__(self):
        super().__init__()
        # Red intermedia: 784 -> 1024 -> 1024 -> 10
        # Parametros de un MLP normal: ~1.8 Millones
        self.layer1 = UnifiedNeuronLayer(784, 1024)
        self.layer2 = UnifiedNeuronLayer(1024, 1024)
        self.layer3 = UnifiedNeuronLayer(1024, 10)
...
def main():
    ...
    print(f"Entrenando V5b (Medium) en: {device}")
    ...
    model = MediumNeuronMLP().to(device)
    
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Parámetros entrenables: {total_params:,} (vs ~1,800,000 de un MLP estándar)")
