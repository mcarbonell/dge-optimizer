"""
dge_neuron_blocks_v41.py
=================================
Experiment to test "Pure Neuron-Blocks" (Structural DGE).
Each block corresponds exactly to a neuron's Fan-In (weights) or its Bias.

Architecture: (784, 128, 64, 10)
Total Blocks: (128 weights + 128 biases) + (64 weights + 64 biases) + (10 weights + 10 biases) = 404 blocks.
...
if __name__ == "__main__":
    ...
    # 2. Neuron-Blocks: 1 block per neuron's fan-in + 1 block per bias
    neuron_layer_sizes = []
    neuron_k_blocks = []
    for a, b in zip(ARCH[:-1], ARCH[1:]):
        # Weights (Fan-In) -> 1 block per output neuron
        neuron_layer_sizes.append(a * b)
        neuron_k_blocks.append(b) # b blocks, each of size a
        # Biases -> 1 block per output neuron
        neuron_layer_sizes.append(b)
        neuron_k_blocks.append(b) # b blocks, each of size 1

    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v41: Pure Neuron-Blocks (Structural DGE)")
    print(f"BUDGET: {BUDGET_DGE:,} evals")
    print(f"Baseline K = {sum(baseline_k_blocks)}")
    print(f"Neuron-Blocks K = {sum(neuron_k_blocks)}")
    print(f"{'='*70}")
    ...
    for seed in SEEDS:
        ...
        # Run Neuron-Blocks
        res_neuron = run_experiment("Neuron_Blocks", model, params0, X_tr_d, y_tr_d, X_te_d, y_te_d, seed, neuron_layer_sizes, neuron_k_blocks)

        summary[seed] = {"baseline": res_base["best_test_acc"], "neuron": res_neuron["best_test_acc"]}

    out_path = Path(__file__).parent.parent / "results" / "raw" / "v41_neuron_blocks.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({"experiment": "v41_neuron_blocks", "results": summary}, f, indent=2)
