"""
dge_neuron_aggregated_v42.py
=================================
Experiment: "One Neuron = One Block".
Weights (Fan-In) and Bias of each neuron are grouped into the same block.

Budget: 500,000 evals
Evaluations per step: 2 * 202 = 404.
Total steps: ~1237.
"""
...
class NeuronOrderedMLP:
    """
    MLP where the parameter vector is ordered by neuron:
    [W1_n0, b1_n0, W1_n1, b1_n1, ..., W2_n0, b2_n0, ...]
    This allows DGE to treat (Fan-In + Bias) as a single contiguous block.
    """
    def __init__(self, arch):
        self.arch = list(arch)
        self.layer_block_sizes = [] # (W + 1) per neuron
    ...
    def forward(self, X, params_batch):
        ...
        for i, (l_in, l_out) in enumerate(zip(self.arch[:-1], self.arch[1:])):
            ...
            # Extract layer chunk: (P, l_out * block_sz)
            layer_p = params_batch[:, offset : offset + sz].view(P, l_out, block_sz)
            
            # W: (P, l_in, l_out), b: (P, 1, l_out)
            # Weights are first l_in params of each block
...
if __name__ == "__main__":
    ...
    # Configuration: Neuron-Aggregated
    # Layer 1: 128 neurons * 785 params
    # Layer 2: 64 neurons * 129 params
    # Layer 3: 10 neurons * 65 params
    layer_sizes = model.layer_param_counts
    k_blocks = [128, 64, 10]

    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v42: Neuron-Aggregated (1 Neuron = 1 Block)")
    ...
    # Init Weights (He/Kaiming) and Biases (0)
    offset = 0
    ...
    res = run_experiment("Neuron_Aggregated", model, params0, X_tr_d, y_tr_d, X_te_d, y_te_d, seed, layer_sizes, k_blocks)

    out_path = Path(__file__).parent.parent / "results" / "raw" / "v42_neuron_aggregated.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({"experiment": "v42_neuron_aggregated", "seed": seed, "acc": res["best_test_acc"]}, f, indent=2)
