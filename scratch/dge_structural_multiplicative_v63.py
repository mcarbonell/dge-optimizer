"""
dge_structural_multiplicative_v63.py
=================================
Experiment to test MULTIPLICATIVE Node Perturbation via Structural DGE.
Instead of adding +delta, we MULTIPLY the weights of a fan-in/fan-out block
by (1 + delta) and (1 - delta).

The hypothesis is that multiplicative perturbation avoids the "Gradient Washing"
issue of additive perturbation because the effective perturbation depends on 
the magnitude (and sign) of the current weight, preserving the relative 
...
# ---------------------------------------------------------------------------
# Structural DGE Optimizer (Multiplicative)
# ---------------------------------------------------------------------------

class StructuralMultiplicativeOptimizer:
    def __init__(
        self,
        arch: list[int],
        dim: int,
        lr: float = 0.05,
        delta: float = 1e-3, # 1.001 multiplier
        beta1: float = 0.9,
        beta2: float = 0.999,
        eps: float = 1e-8,
    ...
    def step(self, f_batched, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        # Multiplicative perturbation: X_batch = x * (1 +/- P_base * delta)
        ...
        # diffs is the gradient with respect to the multiplicative scale of each block.
        # Distribute block scalar gradients back to parameters
        grad_scale = diffs @ self.P_base
        
        # The effective gradient on the weight is grad_scale * weight
        # This reflects the chain rule: dL/dw = dL/dscale * dscale/dw = dL/dscale * (1) -> wait.
        # Actually, if we perturbed by w*(1+delta), the effective delta is w*delta.
        # So grad_w = grad_scale * w.
        grad = grad_scale * x
...
ARCH          = (784, 128, 64, 10)
BUDGET_DGE    = 2500_000
LR_ZO         = 0.01  # Lower LR might be better for multiplicative
DELTA         = 1e-3  # Multiplier is (1 + 1e-3)
BATCH_SIZE    = 256
SEEDS         = [42]
LOG_INTERVAL  = 25_000
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v63: Multiplicative Structural DGE")
    ...
    METHODS = [
        ("DGE_Multiplicative", StructuralMultiplicativeOptimizer)
    ]
    ...
    out_path = out_dir / "v63_multiplicative_structural.json"
    
    payload = {
        "experiment": "v63_multiplicative_structural",
