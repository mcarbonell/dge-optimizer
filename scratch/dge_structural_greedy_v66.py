...
TWIST (v66): Adaptive Deltas per block! (1/5th Success Rule)
Instead of a fixed global delta that causes "ant steps", each block tracks
its own delta_add and delta_mul. 
- If a block perturbation succeeds -> delta *= 1.2 (Accelerate)
- If a block perturbation fails   -> delta *= 0.95 (Fine-tune)

This allows the Greedy local search to automatically find the optimal step
size for every single neuron, dynamically breaking out of plateaus.
"""
...
# ---------------------------------------------------------------------------
# Greedy Adaptive Structural Optimizer
# ---------------------------------------------------------------------------

class GreedyAdaptiveOptimizer:
    def __init__(
        self,
        arch: list[int],
        dim: int,
        delta_add_init: float = 1e-4, 
        delta_mul_init: float = 1e-3, 
        success_factor: float = 1.20,
        fail_factor: float = 0.95,
    ...
    def step_greedy(self, f_batched, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        for b_idx in perm:
            ...
            new_delta = max(self.min_delta, min(self.max_delta, new_delta))
...
ARCH          = (784, 128, 64, 10)
BUDGET_DGE    = 2500_000
DELTA_ADD_0   = 1e-3  # Starting a bit higher like in v65b
DELTA_MUL_0   = 1e-3  
BATCH_SIZE    = 256
SEEDS         = [42]
LOG_INTERVAL  = 50_000
...
def run_dge(name, opt_cls, model, params0, X_tr, y_tr, X_te, y_te, seed):
    ...
    # Let's print out some stats about the deltas at the end to see evolution!
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"EXPERIMENTO v66: Greedy ADAPTIVE Structural DGE")
    print(f"NO EMA. Immediate updates. Adaptive Deltas (Success=x1.2, Fail=x0.95)")
    ...
    METHODS = [
        ("DGE_Greedy_Adaptive", GreedyAdaptiveOptimizer)
    ]
    ...
    out_path = out_dir / "v66_greedy_adaptive_structural.json"
    
    payload = {
        "experiment": "v66_greedy_adaptive_structural",
