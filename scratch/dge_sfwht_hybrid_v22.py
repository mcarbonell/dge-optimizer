...
class HybridSFWHT_DGE:
    def __init__(self, sizes, B_list, top_k_buckets, dge_blocks, total_steps, lr0=0.05, eps0=5e-3, delta0=1e-3):
        self.sizes = sizes
        self.B_list = B_list
    ...
    def estimate_layer_gradient(self, f_layer, x_layer, D_pad, actual_size, B, num_top_buckets, k_blocks, eps, delta):
        # ==========================================
        # PHASE 1: SFWHT Scan (Radar)
        # ==========================================
        ...
        # Select top active buckets
        _, top_buckets = torch.topk(torch.abs(U_base), num_top_buckets)
        
        # Map back from buckets to variable indices
        indices = torch.arange(D_pad, device=device)
        bucket_ids = indices % B
        
        # Get active variable indices
        mask = torch.isin(bucket_ids, top_buckets)
        active_indices = indices[mask]
        
        # Filter out padding indices
        active_indices = active_indices[active_indices < actual_size]
        
        # ==========================================
        # PHASE 2: DGE Refinement (Precise Gradient)
        # ==========================================
        grad = torch.zeros(actual_size, device=device)
        
        if len(active_indices) > 0:
            # Shuffle candidate indices and partition into k_blocks
            perm = active_indices[torch.randperm(len(active_indices), device=device)]
            
            # Ensure we don't have more blocks than variables
            actual_k_blocks = min(k_blocks, len(perm))
            blocks = torch.tensor_split(perm, actual_k_blocks)
            ...
            # Evaluate all DGE block perturbations in one batched forward pass
            batch_x = x_layer.unsqueeze(0) + dge_perts
            Y_dge = f_layer(batch_x)
            evals += num_perts
            
            # Construct gradient for active variables
    ...
    def step(self, f_eval, params):
        ...
        offset = 0
        for i, (size, pad_size, B, num_top_buckets, k_blocks) in enumerate(zip(
            self.sizes, self.padded_sizes, self.B_list, self.top_k_buckets, self.dge_blocks)):
            ...
            g_layer, evs = self.estimate_layer_gradient(f_layer, x_layer, pad_size, size, B, num_top_buckets, k_blocks, eps, delta)
            full_grad[offset:offset+size] = g_layer
            total_evals += evs
            offset += size
            
        # Adam Update (Variables not evaluated get exactly 0 grad, their EMA decays naturally)
...
if __name__ == "__main__":
    print("DGE v22: HYBRID SFWHT + DGE on MNIST")
    ...
    B_LIST = [512, 128]
    TOP_K_BUCKETS = [50, 12]  # ~10% active buckets
    DGE_BLOCKS = [8, 4]       # Granularity for the selected active variables
    
    # Evals L1: 1024 (Scan) + 16 (Refine) = 1040
    # Evals L2: 256 (Scan) + 8 (Refine) = 264
    # Total Evals/Step: ~1304
    ESTIMATED_EVALS_PER_STEP = 2 * B_LIST[0] + 2 * DGE_BLOCKS[0] + 2 * B_LIST[1] + 2 * DGE_BLOCKS[1]
    total_steps = TOTAL_BUDGET // ESTIMATED_EVALS_PER_STEP
    print(f"Config: B={B_LIST}, Top_Buckets={TOP_K_BUCKETS}, DGE_Blocks={DGE_BLOCKS}")
    print(f"Estimated Evals/Step: {ESTIMATED_EVALS_PER_STEP}")
    print(f"Total Steps (Adam updates) for budget {TOTAL_BUDGET}: ~{total_steps}")
    ...
    while evals < TOTAL_BUDGET:
        ...
        # Log every ~10 steps or at the end
        if steps % 10 == 0 or evals >= TOTAL_BUDGET:
            with torch.no_grad():
    ...
    with open(f"results/raw/sfwht_hybrid_v22.json", "w") as f_out:
